...
import { DownloadIcon, FlagIcon, NoteIcon, PinIcon, PromptIcon, ShareIcon, UserIcon } from "../../components/icons";
...
export function MenuItem({ icon, text, underlineOff, action, color, onClick } : { icon: ReactNode, text: string, color?: string, onClick?: () => void,  underlineOff?: boolean, action?: ReactNode }) {
  ...
  return (
    <div style={{color}} onClick={onClick} className={`flex justify-between py-2 px-4 border-primary-8 text-primary-6 h-10 cursor-pointer ${!underlineOff ? "border-b-[1px]" : ""}`}>
